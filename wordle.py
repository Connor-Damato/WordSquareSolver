...
def main():
    ...
    for letter in necessaryLetters:
        print('Input the locations the letter '+letter+' cannot be (separated by \',\' no space):')
...
def getAnswers(currentGuess, lettersToRemove, necessaryLetters, letterDict):
    ...
    for word in file.read().split():
        if (len(word) != 5):
            continue
        if compare(currentGuess, word.lower(), lettersToRemove, necessaryLetters, letterDict):
